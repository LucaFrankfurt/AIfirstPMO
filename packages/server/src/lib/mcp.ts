...
import {
  ...
  dueOn, formatMeasure, parseMeasure, progressOf, trendOf,
 ...
 */
function requireFeature(workspaceId: string, name: 'time' | 'budget' | 'infrastructure' | 'kpi'): void {
...
const visibleKpis = (workspaceId: string, ctx: McpCtx): Row[] =>
...
function findKpi(ref: string, workspaceId: string, ctx: McpCtx): Row {
  const wanted = ref.trim().toLowerCase();
...
const kpiChildren = (table: 'kpi_targets' | 'kpi_readings', kpis: Row[]): Row[] => (kpis.length
  ? all<Row>(
...
const asKpi = (row: Row): Kpi => serialize('kpi', row) as unknown as Kpi;

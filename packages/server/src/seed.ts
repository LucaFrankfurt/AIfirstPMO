 ...
 * view populated on first launch.
 */
...
function seed(): void {
  if (get(`SELECT id FROM workspaces LIMIT 1`)) {
    console.log('Database already contains a workspace — skipping seed.');
    return;
  }
...
`,
  ...
  console.log('Sign in with ada@kolibri.dev / kolibri-demo');
}

...
const en = {
  ...
  /* The nightly backup, when it goes to an address. Read by whoever runs the
     server, often weeks after they set it up — so each one says what it holds,
     how to put it back, and what it costs to have it lying in a mailbox. */
  'backup.subject': 'Kolibri backup {name} — {instance}',
  'backup.body': 'Attached is the snapshot of {instance} taken {when}: {counts}.',
  'backup.howToRestore': 'To put it back: Settings → Data → Restore from a file, and choose this attachment.',
  'backup.filesInBucket': 'Uploaded files are not in the attachment. They are in {bucket}, and a restore on an instance with that bucket set up fetches them from there.',
  'backup.filesInDirectory': 'Uploaded files are not in the attachment; the snapshot in {dir} on the server has them.',
  'backup.filesInStore': 'Uploaded files are not in the attachment; they stay in the object store this instance uses.',
  'backup.filesNowhere': 'Uploaded files are not in the attachment, and no bucket is set up to keep them: restoring this brings back every table and none of the files.',
  'backup.bucketFailed': 'Tonight’s copy to {bucket} did not arrive: {problem}',
  'backup.sensitive': 'The attachment is the whole database — every workspace and every account. Anybody who can read this mailbox can read all of it.',
  'backup.tooLargeSubject': 'Kolibri backup {name} — too large to email',
  'backup.tooLarge': 'Tonight’s snapshot of {instance} is {size}, more than the {limit} this instance sends by email, so it is not attached.',
  'backup.refusedSubject': 'Kolibri backup {name} — refused by the mail provider',
  'backup.refused': 'Tonight’s snapshot of {instance} could not be sent as an attachment. The mail provider said: {reason}',
  'backup.keptInDirectory': 'It is kept in {dir} on the server.',
  'backup.keptInBucket': 'It is in {bucket}.',
  'backup.keptNowhere': 'Nothing else keeps a copy of it. Set up a bucket in Settings → Server → Backups, or raise KOLIBRI_BACKUP_EMAIL_MAX_MB if the mail provider takes more.',
  'backup.failedSubject': 'Kolibri backup failed — {instance}',
  'backup.failed': 'The backup of {instance} at {when} did not complete: {problem}',
  'backup.failedNext': 'Nothing was sent anywhere tonight. Settings → Data lists the backups and has a button to try again.',
  'backup.testSubject': 'Kolibri backup test — {instance}',
  'backup.testBody': 'This is how the nightly backup of {instance} will arrive: as a .zip attached to a message like this one. This one holds only a note. If it reached you with its attachment, the mail provider accepts it.',
  'backup.testFile': 'A test attachment from Kolibri. The real one holds kolibri.sqlite and manifest.json.',
...
const de: Catalogue = {
  ...
  'backup.subject': 'Kolibri-Backup {name} — {instance}',
  'backup.body': 'Im Anhang liegt der Snapshot von {instance} vom {when}: {counts}.',
  'backup.howToRestore': 'Zum Zurückspielen: Einstellungen → Daten → Aus einer Datei wiederherstellen, und diesen Anhang wählen.',
  'backup.filesInBucket': 'Hochgeladene Dateien sind nicht im Anhang. Sie liegen in {bucket}, und ein Restore auf einer Instanz mit diesem Bucket holt sie von dort.',
  'backup.filesInDirectory': 'Hochgeladene Dateien sind nicht im Anhang; der Snapshot in {dir} auf dem Server enthält sie.',
  'backup.filesInStore': 'Hochgeladene Dateien sind nicht im Anhang; sie bleiben im Objektspeicher dieser Instanz.',
  'backup.filesNowhere': 'Hochgeladene Dateien sind nicht im Anhang, und kein Bucket bewahrt sie auf: Wer das zurückspielt, bekommt jede Tabelle und keine der Dateien.',
  'backup.bucketFailed': 'Die Kopie von heute Nacht nach {bucket} ist nicht angekommen: {problem}',
  'backup.sensitive': 'Der Anhang ist die ganze Datenbank — jeder Workspace und jedes Konto. Wer dieses Postfach lesen kann, kann all das lesen.',
  'backup.tooLargeSubject': 'Kolibri-Backup {name} — zu groß für eine E-Mail',
  'backup.tooLarge': 'Der Snapshot von {instance} ist heute Nacht {size} groß, mehr als die {limit}, die diese Instanz per E-Mail verschickt — er hängt deshalb nicht an.',
  'backup.refusedSubject': 'Kolibri-Backup {name} — vom Mailanbieter abgelehnt',
  'backup.refused': 'Der Snapshot von {instance} ließ sich heute Nacht nicht als Anhang verschicken. Der Mailanbieter sagte: {reason}',
  'backup.keptInDirectory': 'Er liegt in {dir} auf dem Server.',
  'backup.keptInBucket': 'Er liegt in {bucket}.',
  'backup.keptNowhere': 'Sonst bewahrt ihn nichts auf. Unter Einstellungen → Server → Backups lässt sich ein Bucket einrichten — oder KOLIBRI_BACKUP_EMAIL_MAX_MB erhöhen, falls der Mailanbieter mehr annimmt.',
  'backup.failedSubject': 'Kolibri-Backup fehlgeschlagen — {instance}',
  'backup.failed': 'Das Backup von {instance} am {when} ist nicht fertig geworden: {problem}',
  'backup.failedNext': 'Heute Nacht wurde nichts verschickt. Einstellungen → Daten zeigt die Backups und hat einen Knopf, es noch einmal zu versuchen.',
  'backup.testSubject': 'Kolibri-Backup-Test — {instance}',
  'backup.testBody': 'So kommt das nächtliche Backup von {instance} an: als .zip im Anhang einer Nachricht wie dieser. Diese hier enthält nur eine Notiz. Ist sie mit Anhang angekommen, nimmt der Mailanbieter ihn an.',
  'backup.testFile': 'Ein Test-Anhang von Kolibri. Der echte enthält kolibri.sqlite und manifest.json.',
...
const fr: Catalogue = {
  ...
  'backup.subject': 'Sauvegarde Kolibri {name} — {instance}',
  'backup.body': 'Ci-joint l’instantané de {instance} pris le {when} : {counts}.',
  'backup.howToRestore': 'Pour le remettre en place : Réglages → Données → Restaurer depuis un fichier, puis choisir cette pièce jointe.',
  'backup.filesInBucket': 'Les fichiers envoyés ne sont pas dans la pièce jointe. Ils sont dans {bucket}, et une restauration sur une instance configurée avec ce bucket les y récupère.',
  'backup.filesInDirectory': 'Les fichiers envoyés ne sont pas dans la pièce jointe ; l’instantané dans {dir} sur le serveur les contient.',
  'backup.filesInStore': 'Les fichiers envoyés ne sont pas dans la pièce jointe ; ils restent dans le stockage objet de cette instance.',
  'backup.filesNowhere': 'Les fichiers envoyés ne sont pas dans la pièce jointe, et aucun bucket ne les conserve : restaurer ceci ramène toutes les tables et aucun des fichiers.',
  'backup.bucketFailed': 'La copie de cette nuit vers {bucket} n’est pas arrivée : {problem}',
  'backup.sensitive': 'La pièce jointe est la base de données entière — chaque espace de travail et chaque compte. Quiconque peut lire cette boîte peut tout lire.',
  'backup.tooLargeSubject': 'Sauvegarde Kolibri {name} — trop volumineuse pour un e-mail',
  'backup.tooLarge': 'L’instantané de {instance} fait {size} cette nuit, plus que les {limit} que cette instance envoie par e-mail ; il n’est donc pas joint.',
  'backup.refusedSubject': 'Sauvegarde Kolibri {name} — refusée par le fournisseur de messagerie',
  'backup.refused': 'L’instantané de {instance} n’a pas pu être envoyé en pièce jointe cette nuit. Le fournisseur a répondu : {reason}',
  'backup.keptInDirectory': 'Il est conservé dans {dir} sur le serveur.',
  'backup.keptInBucket': 'Il est dans {bucket}.',
  'backup.keptNowhere': 'Rien d’autre n’en garde de copie. Configurez un bucket dans Réglages → Serveur → Sauvegardes, ou augmentez KOLIBRI_BACKUP_EMAIL_MAX_MB si le fournisseur accepte davantage.',
  'backup.failedSubject': 'Échec de la sauvegarde Kolibri — {instance}',
  'backup.failed': 'La sauvegarde de {instance} du {when} n’a pas abouti : {problem}',
  'backup.failedNext': 'Rien n’a été envoyé cette nuit. Réglages → Données liste les sauvegardes et propose de réessayer.',
  'backup.testSubject': 'Test de sauvegarde Kolibri — {instance}',
  'backup.testBody': 'Voici comment la sauvegarde nocturne de {instance} arrivera : un .zip joint à un message comme celui-ci. Celui-ci ne contient qu’une note. S’il vous est parvenu avec sa pièce jointe, le fournisseur l’accepte.',
  'backup.testFile': 'Une pièce jointe de test de Kolibri. La vraie contient kolibri.sqlite et manifest.json.',

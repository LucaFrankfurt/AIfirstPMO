...
/**
 * The directory the workspace lives in — found, not counted.
 *
 * It was `resolve(here, '../../..')`, which was right for as long as this file
 * sat at `packages/server/src/env.ts` and wrong the moment it moved two
 * directories deeper. Nothing said so: `ROOT` only feeds defaults that a
 * deployment overrides anyway, so the typechecker was happy, every test passed,
 * and the first thing that noticed was the browser walkthrough finding no web
 * build. A path that counts `..` is a path that breaks silently when a file is
 * moved, so this looks for the marker instead.
 */
...
export const ROOT = resolve(workspaceRoot(here));

...
- **MCP natively.** An assistant is a user with a scoped token, not a plugin or a chat sidebar. 117
  tools over the same permissions as a person, and a read-only token is refused by all 63 that

...
| `list_products` | the catalogue with the four figures it is read for: the price, what one unit costs, the margin, and how much has to be sold to break even. A product nobody has priced comes back with `price: null` rather than zero — a model reading `0` would say the product is free. `terms` appears when the same offer is sold on more than one commitment: `periods` says how often it is billed, which for three subscription prices is "monthly" three times over, and the figures beside it quote the shortest term — the dearest of them |
...
| `delete_task` | soft delete, flagged `destructiveHint` for clients that confirm — as is every tool that loses something, which is a flag on the tool now rather than one tool's name |
...
| `create_page` / `update_page` | `update_page` takes `content` (replace) or `append`, and `parent` to move it in the tree (`root` for the top level) — a page offered its own subtree is refused. A `title` change carries the `[[links]]` that pointed at the old one and answers `links_followed` with how many pages moved. Both take `format`: `markdown` (the default) or `html`, which is rendered through an allowlist — a `<script>` in it is dropped at render rather than stored and refused. `update_page` changes the flag and does **not** convert the body, so send `content` in the new language in the same call |
...
| `update_product` | anything about a product that is not a price, a cost, a person or a part. Only the fields named are touched, and a product still holding parts is refused back to `single`: `unitCosts` recurses through them whatever `kind` says. `currency` is refused unless it is three capitals — the write path would otherwise snap it to EUR and restate every amount — and a change to it names the packages now summing two currencies under one symbol |
| `update_product_price` | **corrects** a price in place, losing what it said. Changing what something costs from a date onward is `change_product_price`, which keeps the old amount as history — the description says which is which, because a model picking the wrong one destroys a price history quietly. `packages_affected` names what else moved: `bundleValue` reads each part's applicable price, so an amount, a window or a threshold changed here changes what every package holding it is worth |
| `update_product_cost` | the one this group was missing. Moving a cost between bases moves money between the margin and the break-even, so the answer carries the new unit cost, per-delivery cost, contribution and margin rather than only the row — and `packages_affected`, because `unitCosts` recurses into parts and a module's cost is a package's cost |
| `update_product_contributor` | what an outside contributor is paid, or on what basis. A fee is a cost on exactly its basis, so the answer names the packages it moved too |
| `update_product_part` | how many of a product a package holds, or which product it is. A package may not end up inside itself at any depth — `refuseCycle` runs on updates too, falling back to the existing row for whichever side the patch does not name. A package may hold the same product twice, so a name matching two rows is a question, asked rather than guessed |
| `update_promotion` | a campaign's discount, window, coverage, spend, uplift or status. `value` is read against the kind the campaign *will* have, not the one it had, because the field means three things: basis points for `percent`, what comes off for `amount`, what it becomes for `price`. A `kind` moved without a `value` is refused for that reason — the stored number would survive and mean something else |
| `delete_product` | soft-deletes it and everything that goes with it, and says what the packages holding it lost. Counted before the write, because the cascade runs in the same transaction. Saved simulations are *not* in the cascade, so the answer names the ones now pointing at nothing |
| `delete_product_price` | for a price that should never have existed. The answer says what applies now, because removing the row that applied today leaves a product reading "unpriced" everywhere — and which packages just lost its value, since `bundleValue` counts an unpriced part as worth nothing rather than as missing |
...
| `remove_product_part` | take a product out of a package. The package keeps its price and its kind; the answer says how many are left, so an empty one is visible rather than discovered. A name matching two of the same part is refused, because which row is destroyed is not ours to guess |

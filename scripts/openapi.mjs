  ...
  return {
    ...
    info: {
      ...
      description: [
        ...
        '`npm run openapi -- --fix` rewrites it, and `npm run check:openapi` fails if it has drifted from the tree.',
        '',
...
if (before === null) {
  console.error(`${OUT} does not exist. Run \`npm run openapi -- --fix\`.`);
  process.exit(1);
}
if (before !== rendered) {
  console.error(`${OUT} no longer matches the tree. Run \`npm run openapi -- --fix\`.`);
  process.exit(1);
}

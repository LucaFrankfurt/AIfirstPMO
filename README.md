...
| **Moving house** | Snapshots taken nightly, verified before the older ones are pruned, and sent where you say — an S3 bucket of their own, files included, or an email address, set up in the app with nothing mounted — and put back **from the app**, while it is running: deploy a Kolibri somewhere new, give it the same bucket (or upload the snapshot), sign in with your old password, and it *is* the old instance. Accounts, workspaces, files and settings included. It replaces the tables in one transaction rather than swapping the file, so an older snapshot still restores and what it replaces is snapshotted first. [Backups](docs/deployment.md#backups) |
...
| **Rates, cost and a timesheet** | A week of logged time laid out day by day, per person or per project, and what it cost. Hourly rates are **dated rather than current** — raising one in April does not restate March, because an hour is costed at whatever was in force the day the work happened — and the most specific of them wins: this person on this project, then this person anywhere, then anybody on this project, then the workspace's own. Two kinds, what an hour costs and what it is charged at, so cost, revenue, margin and billable share are all answerable. Hours no rate covers are reported as **unrated**, never as free. Rates and every figure derived from one are owners' and admins' only — a total is a rate anybody can divide back out. [Time and rates](docs/time.md) |
| **Infrastructure and vendors** | Servers, the instances on them, SaaS subscriptions and who you buy them from — and **a landscape is a date rather than a document**: every component says when it joined and when it leaves, so "what runs today" and "what runs next March" are the same screen asked twice, and the future never goes stale. Pick two dates and get what is gone, what has arrived, and what the difference costs a year. Documented moves between one landscape and the next name what they retire and what they bring in, so progress is read from the register rather than from a status — a move claimed done with a server still running is flagged. Contracts carry a notice period, because the day you stop being able to leave is the one people miss. A component charged to a budget line puts what the register says beside what was planned. [The register](docs/infrastructure.md) |
| **Budgets** | What things cost, planned against what has actually gone. Infrastructure, investments, licences, people and the rest, each planned once — a monthly hosting bill is one line, not twelve — and **split across the projects that pay for it**, so a shared cluster charges 60% to one team and 40% to another and both figures are right. Committed spend counts as gone, because a purchase order is money you no longer have. Closing a month is one press per line — the actuals screen offers what the plan says is due, at the line's own day of the month, and refuses to offer a line twice. A forecast that takes closed months as they happened and the plan for the rest, a run-rate second opinion beside it, and scenarios that never touch the plan: slip an investment a quarter, cut a category by a third, or carry only the money somebody has signed for. Money is stored in whole minor units and one currency per budget — nothing here invents an exchange rate. Off by default; switched on per workspace. [Budgets](docs/budgets.md) |
| **Products** | What the organisation sells, and whether selling it is worth doing. A product carries its Umfang — two days, twelve months, forty hours — the capacities it can be delivered at, the capabilities it gives a customer, and the **external speakers and subcontractors** who are part of it, as people with a fee rather than as a cost line named after them. Several prices at once, and which one applies is worked out from the order size and the day: list, a volume tier, a partner price, and an internal transfer price that never counts as revenue. **A package is a product**, so every screen and every total works on it unchanged — and what it would cost bought part by part is computed beside what it actually costs, so the discount is visible and a premium is too. Costs are split by what they vary with, which is the only distinction a break-even is made of, and the break-even says when there *is no answer*: nobody has priced it, every unit loses money, or the arithmetic wants more seats than the room holds. Campaigns are a price that is different for a while, with what running it costs and what it is expected to sell, and whether one is running today is worked out from the dates rather than stored. Simulations project a product forward month by month — capacity is a ceiling rather than a suggestion, acquisition is charged in the month the customer arrives, and a subscription keeps paying and keeps churning. Retention is three fields and no customer register: what a customer is worth net of winning them, and how long until that is back. Off by default; switched on per workspace. [Products](docs/products.md) |
| **KPIs** | Numbers somebody has undertaken to watch — uptime, churn, lead time — with what they have to reach and **by which milestone**: a target hung on a milestone takes that milestone's date live, so a slipped release moves the deadline rather than turning it into a miss. Values are whole numbers at a scale you set, like money, because they get averaged and compared. "On track" means past a straight line from the baseline to the target, not near the target — and the three states a dashboard usually paints green by omission are counted as themselves: nothing measured, nothing promised, and **a reading too old to stand for today**. Off by default; switched on per workspace. [KPIs](docs/kpi.md) |
| **Decisions** | A question with options on it: one choice or several, until Friday or until somebody closes it, and a record of what the team picked once it is over. **A secret ballot is secret at the pull** rather than on the screen — nobody's device receives anybody else's vote, so the count comes off counters the write path keeps instead of out of rows a browser could read. Every share is of the *people who voted* rather than of the ticks, which is the same figure in a single-choice vote and the only readable one when several may be picked, and a tie is reported as both options because that is the moment somebody has to talk. Nothing runs when a deadline passes: a vote past it reads as **expired** rather than closed, which is a different sentence. Attach one to a task and it stays when the task goes. **Opening one tells everybody who can answer it** — once the question is actually answerable, never the author, never a guest, and never somebody who cannot see the project it was taken in. Off by default; switched on per workspace. [Decisions](docs/decisions.md) |
| **Connected mailboxes** | Shared inboxes — `support@`, `info@`, `admin@` — connected once and searchable from one place *and from an assistant*, which is the query a mail client cannot run because it is pointed at one account at a time. **Read-only by construction, not by promise**: the IMAP session selects with `EXAMINE`, so the server itself refuses to mark, move or delete anything. Words match subjects, bodies and **attachment names**, in a small German-and-English dialect — `von:stripe seit:2024-01 rechnung`. A document hunt ranks messages by how likely each is to carry an invoice, a receipt or a statement and returns **the evidence rather than only a score**, because what counts as tax-relevant is a judgement about a business and a filter that quietly drops an invoice is worse than none. Numbers over the whole estate of inboxes: volume by mailbox and month, top senders and domains, reply times — each carrying the window the copy actually covers. Per mailbox, either the whole workspace or exactly the people named, where an **empty list means nobody** rather than everybody. Sign in with a password, or with **OAuth** where an app password is awkward or forbidden by policy — Google and Microsoft 365, the token refreshed on its own and stored where the password would have been. Found by the one search box over everything too, by registering as a corpus rather than by the kernel learning what a mailbox is. The credential is not on the row and never on the wire. Off by default; switched on per workspace. [Mailboxes](docs/mail.md) |
...
| **On a phone** | The same bundle inside a Capacitor shell, for the App Store and Google Play — one client, so a screen fixed once is fixed in three places. A packaged app loads from its own origin rather than the server's, so it asks once which Kolibri it belongs to and carries a bearer token instead of a cookie; a browser keeps its `HttpOnly` cookie and is never asked. [Building one](docs/mobile.md) |
...
| **Collaboration** | Comments with markdown, attachments and reactions on tasks *and* pages; `@mentions` with autocomplete; following; a per-task activity trail; invite links; roles (owner / admin / member / guest); private projects. One markdown dialect across every box you can write in, escaped before anything is rendered rather than sanitised afterwards. [Syntax](docs/markdown.md) |
...
| **Integration** | A REST API for every entity, scoped API tokens, signed outgoing webhooks with a delivery log, retries and a replay button (Slack and Discord shapes too) and incoming ones that link a commit to the task it names, plus an MCP server over HTTP and stdio with 117 tools, 6 prompts and page resources. [API](docs/api.md) · [OpenAPI](docs/openapi.json) · [MCP](docs/mcp.md) · [n8n](docs/n8n.md) |
...
| Attachments | `upload_attachment`, `list_attachments`, `get_attachment`, `delete_attachment` |
...
| KPIs | `list_kpis`, `kpi_status`, `create_kpi`, `record_measurement`, `set_kpi_target` |
| Decisions | `list_decisions`, `decision_result`, `create_decision`, `cast_vote`, `close_decision` |
| Budgets | `list_budgets`, `budget_status`, `create_budget`, `add_budget_line`, `record_spend`, `confirm_planned`, `project_costs` |
...
**6 prompts**: `weekly_review`, `meeting_notes`, `standup`, `sprint_planning`, `triage` — the list a
...
| [`infrastructure.md`](docs/infrastructure.md) | The estate, why a landscape is a date rather than a document, and how a move is checked against the register |
| [`kpi.md`](docs/kpi.md) | Numbers you have undertaken to watch: the one rule behind "on track", why staleness is a state of its own, and what a target due by a milestone does when the milestone moves |
| [`decisions.md`](docs/decisions.md) | A question with options on it: why a secret ballot is a clause in the sync filter rather than a hidden column, why the share is of the people rather than of the ticks, and why nothing runs when a deadline passes |
| [`mail.md`](docs/mail.md) | Connected inboxes: what is copied and what is not, why read-only is a property of the session rather than a rule, the one place an empty member list means nobody, and how the document hunt ranks without deciding |
...
| [`products.md`](docs/products.md) | What is sold: why a package is a product, which of several prices applies, the three ways a break-even has no answer, and what retention is honestly claiming |
...
npm run check:responsive  # 27 screens, 340px to 1600px in 20px steps, looking for overflow

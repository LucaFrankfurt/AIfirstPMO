...
| **Integration** | A REST API for every entity, scoped API tokens, signed outgoing webhooks with a delivery log, retries and a replay button (Slack and Discord shapes too) and incoming ones that link a commit to the task it names, plus an MCP server over HTTP and stdio with 80 tools, 6 prompts and page resources. [API](docs/api.md) · [MCP](docs/mcp.md) · [n8n](docs/n8n.md) |
...
| Mail *(read-only but one)* | `list_mailboxes`, `search_mail`, `get_mail`, `mail_thread`, `find_documents`, `list_mail_attachments`, `mail_stats`, `file_mail_as_task` |

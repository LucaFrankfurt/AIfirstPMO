...
| **Pages** | Nested markdown wiki with version history, restore and a what-changed diff; labels and filtering; watch a page; page templates; per-page visibility; export as a markdown bundle; print or save as PDF; read-only share links for people outside the workspace; comments and `@mentions`, including inline comments on a selected passage that survive the text being edited around them; drag & drop images |

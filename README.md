...
| **Integration** | A REST API for every entity, scoped API tokens, signed outgoing webhooks with a delivery log, retries and a replay button (Slack and Discord shapes too) and incoming ones that link a commit to the task it names, plus an MCP server over HTTP and stdio with 66 tools, 5 prompts and page resources. [API](docs/api.md) · [MCP](docs/mcp.md) · [n8n](docs/n8n.md) |
